...
#### Authentication (`/auth`)
- `POST /auth/create-account` - Register new user account
...
- `POST /rooms/add/new-room` - Add new room (admin)
- `PUT /rooms/update/room/{roomId}` - Update room details (admin)
- `DELETE /rooms/delete/room/{roomId}` - Delete room (admin)

#### Bookings (`/rooms`)
- `POST /rooms/browse-rooms/booking/{roomId}` - Book a room (authenticated users)
